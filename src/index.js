...
(async() => {
  try {
    ...
    const manifestData = { destination, incremental: IS_INCREMENTAL, primary_key: PRIMARY_KEY };
    ...
    if (size(files) > 0) {
      const transferedFiles = await transferFilesFromSourceToDestination(tmpDir, tableOutDir, files, fileName, reportType, getKeysBasedOnReportType(reportType));
